...
- `KOMGA_CONFIG` (default `./config/komga`): persists Komga DB/thumbnails.
- `KOMGA_TMP` (default `/tmp/komga`): temp extraction dir.
- `KOMGA_PORT` (default 8081): host port mapped to Komga’s internal 25600.
- `KOMF_CONFIG` (default `./config/komf`): Komf application.yml and DB.
...
## Podman notes
- Works with `podman compose` the same way. If using rootless Podman + systemd, generate units via `podman generate systemd --files --name komga` and enable under the user session.
